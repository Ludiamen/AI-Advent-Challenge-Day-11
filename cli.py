...
EPILOG = f"""\
слои памяти (--слои):
  кратко   краткосрочная — реплики текущего диалога, SQLite «память/диалог.db»
  рабочая  рабочая       — состояние текущей задачи, JSON в «память/рабочая/»
  долго    долговременная — профиль, решения и знания в «память/долго/<кто>/»
  ...
  именно даёт каждый из них. Это и делает сравнение.py.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--память-в", dest="base_dir", default="", metavar="ПУТЬ",
                        help="каталог с памятью (по умолчанию «память»)")
