...
class ProfileStore:
    ...
    def load(self) -> dict[str, Any]:
        """Текущий профиль; пустой каркас, если профиля ещё нет."""
        каркас = {
            "user_id": "",
            "стиль": {},
            "ограничения": {},
    ...
    def update(self, section: str, key: str, value: str) -> dict[str, Any]:
        ...
        elif section in ("стиль", "ограничения"):
            if not key:
                raise LongTermError(f"Для раздела «{section}» нужен ключ записи.")
            профиль.setdefault(section, {})[key] = value
        else:
            raise LongTermError(
                f"Неизвестный раздел профиля «{section}». "
                "Допустимы: стиль, ограничения, контекст."
            )
        self.save(профиль)
        return профиль
    ...
    def forget(self, section: str, key: str) -> bool:
        профиль = self.load()
        if section in ("стиль", "ограничения") and key in профиль.get(section, {}):
...
class LongTermMemory:
    ...
    def stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "профиль_заполнен": bool(профиль.get("стиль") or профиль.get("ограничения")),
            "стиль": len(профиль.get("стиль", {})),
